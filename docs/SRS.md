...
| **Luồng thay thế / Quy tắc** | - Slug không tồn tại → API trả 404, lớp `api.ts` đổi thành `null`, trang gọi `notFound()` → giao diện 404.<br>- Sản phẩm **không thuộc danh mục nào** → danh sách liên quan rỗng, khối liên quan không hiển thị.<br>- Danh mục của sản phẩm được sắp theo `Category.position`, nên "danh mục đầu tiên" là danh mục có `position` nhỏ nhất.<br>- Giá đưa vào giỏ là **giá thực tế** `sale_price` nếu có, ngược lại `price`; con số này chỉ để hiển thị — khi đặt hàng backend tính lại (UC-GH-02).<br>- `stock = 0` → card và trang chi tiết hiện nhãn **"Hết hàng"**, nút thêm vào giỏ bị vô hiệu; ô số lượng ở trang chi tiết chặn trên tại `stock`.<br>- **Ô số lượng** (`QuantityInput`, dùng chung cho trang chi tiết, drawer và trang giỏ) gõ được từ bàn phím: `inputMode=numeric`, lọc mọi ký tự không phải chữ số (kể cả khi dán), chặn phím `. , e + -`; giá trị chỉ **chốt khi rời ô / Enter**: rỗng, chữ hoặc `0` → giữ giá trị cũ, vượt trần → kẹp về trần (`stock` ở trang chi tiết, 999 ở giỏ — khớp `zod` của UC-GH-02). Nút `−` khoá tại 1, `+` khoá tại trần. Giỏ cũ (localStorage) còn giữ sản phẩm đã hết thì bị chặn ở bước đặt hàng (UC-GH-02). |
